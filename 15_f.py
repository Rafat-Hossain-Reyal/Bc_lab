...
def generate_proof(word, data_blocks, tree_levels):
    if word not in data_blocks:
        return None, []
    index = data_blocks.index(word)
...
def print_proof(word, word_hash, proof, tree_levels):
    ...
    if proof:
        ...
        print(f"Checking proof for '{word}':")
        index = tree_levels[0].index(word_hash)
        current_index = index
        for i, (sibling_hash, pos) in enumerate(proof):
            sibling_index = pos ^ 1
            if i == 0:
                print(f"h({sibling_index + 1}) = {sibling_hash}")
            else:
                start = (sibling_index * (2 ** i)) + 1
                end = min((sibling_index + 1) * (2 ** i), len(tree_levels[0]))
                nodes = '+'.join(str(j) for j in range(start, end + 1))
                print(f"h({nodes}) = {sibling_hash}")
    else:
        print(f"Word '{word}' is NOT found in the tree.")
